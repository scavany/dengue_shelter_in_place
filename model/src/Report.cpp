...
Report::Report(){
  ...
  reportMaleGenetics = false;
  reportGenetics = false;
  printR0 = false;
  reportSpatial = false;
  ...
  dailyFOI.clear();
  dailyImmature.clear();
  dailyMaleGenetics.clear();
  dailyFemaleGenetics.clear();
...
void Report::setupReport(string file, string outputPath_, string simName_) {
  ...
  reportGenetics = this->readParameter("genetics_print", reportGenetics);
  if (reportGenetics){
    reportMaleGenetics = this->readParameter("genetics_males_print", reportMaleGenetics);
  }
  reportMozAges = this->readParameter("moz_ages_print", reportMozAges);
  reportBites = this->readParameter("bites_print",reportBites);
  reportMozUniqueBites = this->readParameter("moz_unique_bites_print",reportMozUniqueBites);
  ...
  spatialInfLoc = this->readParameter("spatial_infection_location", spatialInfLoc);
  ...
  if (reportGenetics) {
    outputGeneticsFile = outputPath_ + "/" + simName_ + "_genetics.csv";
  }
  ...
  if (reportBites) {
    outputBitesFile = outputPath_ + "/" + simName_ + "_bites.csv";
  }

  if (reportMozUniqueBites) {
    outputUniqueBitesFile = outputPath_ + "/" + simName_ + "_unique_bites.csv";
  }
...
void Report::updateMozUniqueBitesReport(int currDay, Mosquito * m) {
  if (reportMozUniqueBites) {
    mozUniqueBites[m->getNumUniqueBites()][currDay]++;
  }
}  
...
void Report::updateMaleGeneticsReport(int currDay, string locus, string genotype, double number){
  if (reportMaleGenetics){
    dailyMaleGenetics[currDay][locus][genotype] += number;
  }
}

void Report::updateFemaleGeneticsReport(int currDay, string locus, string genotype){
  if (reportGenetics){
    dailyFemaleGenetics[currDay][locus][genotype]++;
  }
}
...
void Report::updateSprayReport(int currDay, string loc,string outbreakName_){
  if (reportSpray){
    spray_counts[outbreakName_][currDay]++;
  }
  if (reportSprayLocs){
    dailySprayLocs[outbreakName_][currDay].push_back(loc);
  }
}

void Report::updateBitesReport(int currDay, int bitesToday) {
  if (reportBites) {
    dailyBites[currDay] = bitesToday;
  }
}
...
void Report::addOutbreakResponseLocations(int currDay, Location * locNow, string outbreakName_){
  dailyOutbreakResponseLocations[outbreakName_][currDay]++;
  if(reportSpatial == true && spatialOutbreakReport){
    string tmp_str = "OutbreakResponseLocation" + outbreakName_ + "," + locNow->getLocID() + "," + std::to_string(currDay) + ",NaN";
    if (spatialInfLoc) {
      tmp_str += (",NaN");
    }
    spatialData.push_back(tmp_str);
...
void Report::addOutbreakSymptomatic(int currDay, Human * h, bool respond_, string outbreakName_){
  dailyOutbreakResponseSymp[outbreakName_][currDay]++;
  if(reportSpatial == true && spatialOutbreakReport){
    string tmp_str = "OutbreakReportedHum" + outbreakName_ + "," + h->getHouseID() + "," + std::to_string(currDay) + ",NaN";
    if (spatialInfLoc) {
      tmp_str += (",NaN");
    }
    spatialData.push_back(tmp_str);
    tmp_str = "OutbreakReportedHumInfLoc" + outbreakName_ + "," + h->getInfLocID() + "," + std::to_string(currDay) + ",NaN";
    if (spatialInfLoc) {
      tmp_str += (",NaN");
    }
    spatialData.push_back(tmp_str);
    if(respond_){
      tmp_str = "OutbreakRespondToHum" + outbreakName_ + "," +  h->getHouseID() + "," + std::to_string(currDay) + ",NaN";
      if (spatialInfLoc) {
	tmp_str += (",NaN");
      }
      spatialData.push_back(tmp_str);
      tmp_str = "OutbreakRespondToHumInfLoc" + outbreakName_ + "," + h->getInfLocID() + "," + std::to_string(currDay) + ",NaN";
      if (spatialInfLoc) {
	tmp_str += (",NaN");
      }
      spatialData.push_back(tmp_str);
...
void Report::printSpatialHeader(){
  if (spatialInfLoc) {
    outSpatial << "Variable,code,Day,Serotype,InfLoc\n";
  } else {
...
void Report::printFOIReport(int lastDay){
  ...
  if(reportFOIOutbreakSymptomatics){
    for (auto it = dailyOutbreakResponseLocations.begin(); it != dailyOutbreakResponseLocations.end(); ++it) {
      headerstr.push_back(it->first+"ReportedSymptomatics");
      headerstr.push_back(it->first+"OutbreakResponseLocations");
    }
  }
  ...
  for(int d = 0; d < lastDay; d++){
	...
	if(dailyOutbreakResponseSymp[it->first].find(d) != dailyOutbreakResponseSymp[it->first].end()){
	  outbreakstr = std::to_string(dailyOutbreakResponseSymp[it->first][d]);
	}
	foi_values.push_back(outbreakstr);
	outbreakstr= "0";
	
	if(dailyOutbreakResponseLocations[it->first].find(d) != dailyOutbreakResponseLocations[it->first].end()){
	  outbreakstr = std::to_string(dailyOutbreakResponseLocations[it->first][d]);
	}
	foi_values.push_back(outbreakstr);
      }
    }
...
void Report::printGeneticsReport(int lastDay){
  outGenetics.open(outputGeneticsFile);
  if (!outGenetics.good()) {
    exit(1);
  }
  std::vector<string> headerstr; string headout;
  headerstr.clear();
  std::vector<string> geno_vec;
  std::vector<string> locus_vec;
  auto firstDay = dailyFemaleGenetics.begin();
  auto firstLocus = (firstDay->second).begin();
  for (auto it = (firstDay->second).begin(); it != (firstDay->second).end(); ++it) {					    
    locus_vec.push_back(it->first);
  }
  for (auto it = (firstLocus->second).begin(); it != (firstLocus->second).end(); ++it) {					    
    geno_vec.push_back(it->first);
  }
  for (auto it = locus_vec.begin(); it != locus_vec.end(); ++it) {
    for (auto it2 = geno_vec.begin(); it2 != geno_vec.end(); ++it2) {
      headerstr.push_back("Females_" + *it + "_" + *it2);
    }
  }
  if (reportMaleGenetics) {
    for (auto it = locus_vec.begin(); it != locus_vec.end(); ++it) {
      for (auto it2 = geno_vec.begin(); it2 != geno_vec.end(); ++it2) {
	headerstr.push_back("Males_" + *it + "_" + *it2);
      }
    }
  }
  ...
  for(int d = 0; d < lastDay; d++){
    string outstring;
    vector<string> genetics_values; genetics_values.clear();
    if(dailyFemaleGenetics.find(d) != dailyFemaleGenetics.end()){
      for (auto itLoc = locus_vec.begin(); itLoc != locus_vec.end(); ++itLoc) {
	for (auto itGen = geno_vec.begin(); itGen != geno_vec.end(); ++itGen) {
	  genetics_values.push_back(std::to_string(dailyFemaleGenetics[d][*itLoc][*itGen]));
	} 
      }
      if (reportMaleGenetics) {
	for (auto itLoc = locus_vec.begin(); itLoc != locus_vec.end(); ++itLoc) {
	  for (auto itGen = geno_vec.begin(); itGen != geno_vec.end(); ++itGen) {
	    genetics_values.push_back(std::to_string(dailyMaleGenetics[d][*itLoc][*itGen]));
	  }
	}
      }
    } else {
      for (auto itLoc = locus_vec.begin(); itLoc != locus_vec.end(); ++itLoc) {
	for (auto itGen = geno_vec.begin(); itGen != geno_vec.end(); ++itGen) {
	  genetics_values.push_back("0");
	} 
      }
      if (reportMaleGenetics) {
	for (auto itLoc = locus_vec.begin(); itLoc != locus_vec.end(); ++itLoc) {
	  for (auto itGen = geno_vec.begin(); itGen != geno_vec.end(); ++itGen) {
	    genetics_values.push_back("0.0000");
	  }
	}
      }
    }
        
    if(!genetics_values.empty()){
      Report::join(genetics_values,',',outstring);
      outGenetics << d << ",";
      outGenetics << outstring;
    }
  }
  outGenetics.close();
}
...
void Report::printSprayReport(int lastDay){
  ...
  if(!headerstr.empty()){
    outSpray << "outbreakResponse,day,";
    Report::join(headerstr,',',headout);
    outSpray << headout;
  }

  for (auto it = spray_counts.begin(); it != spray_counts.end(); ++it) {
    for(int d = 0; d < lastDay; d++){
      string outstring;
      vector<string> spray_values; spray_values.clear();
      
      string sprstr = "0";
      if(spray_counts[it->first].find(d) != spray_counts[it->first].end()){
	sprstr = std::to_string(spray_counts[it->first][d]);
      }
      spray_values.push_back(sprstr);
      
      if(dailySprayLocs[it->first].find(d) != dailySprayLocs[it->first].end()){
	for (auto & loc: dailySprayLocs[it->first][d]){
...
void Report::printBitesReport(int lastDay){
  ...
  for(int d = 0; d < lastDay; d++){
    string outstring;
    vector<string> bites_values; bites_values.clear();
      
    string bitstr = "0";
    if(dailyBites.find(d) != dailyBites.end()){
      bitstr = std::to_string(dailyBites[d]);
    }
    bites_values.push_back(bitstr);
    if(!bites_values.empty()){
      Report::join(bites_values,',',outstring);
      outBites << d << ",";
      outBites << outstring;
    }
  }
...
void Report::printMozUniqueBitesReport(int lastDay) {
  outUniqueBites.open(outputUniqueBitesFile);
  if (!outUniqueBites.good()) {
    exit(1);
  }
  //  for (const auto &e: mozUniqueBites) outUniqueBites << e << "\n";
  std::vector<string> headerstr;
  string headout;
  headerstr.clear();
  headerstr.push_back("NumBites");
  for (int d=0; d<lastDay; d++) {
    headerstr.push_back("Day " + std::to_string(d));
  }
  if(!headerstr.empty()){
    Report::join(headerstr,',',headout);
    outUniqueBites << headout;
  }
  for (auto it = mozUniqueBites.begin(); it != mozUniqueBites.end(); ++it) {
    string outstring;
    vector<string> unique_bites_values; unique_bites_values.clear();
    unique_bites_values.push_back(std::to_string(it->first));
    for(int d = 0; d < lastDay; d++){
      string uniquebitesstr = "0";
      if(it->second.find(d) != it->second.end()){
	uniquebitesstr = std::to_string(it->second[d]);
      }
      unique_bites_values.push_back(uniquebitesstr);
    }
    if(!unique_bites_values.empty()){
      Report::join(unique_bites_values,',',outstring);
      outUniqueBites << outstring;
    }
  }
  outUniqueBites.close();
}
