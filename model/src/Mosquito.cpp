			...
			double mozEIP)
{
  ...
  if(infection == nullptr){
    if(locNow->getInfectedVisitor()){
      ...
      if(humBite != nullptr){
	bitees.insert(humBite->getPersonID());
      }
      return true;
    }
  }
...
//each bool in the pair refers to the allele
void Mosquito::setGenotype(string locus, std::pair<bool, bool> genotype) {
  genotypes[locus] = genotype;
}

std::pair<bool, bool> Mosquito::getGenotype(string locus) {
