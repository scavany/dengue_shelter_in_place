...
void Human::setShelterInPlace() {
  todayTraj.clear();
  path_t temp_t = std::make_pair(houseID,1.0);
  todayTraj.push_back(temp_t);
}

void Human::setFeverEffect(bool mvmtA, double oddsRatio,
			   double locsProp, RandomNumGenerator * rGen) {
  todayTraj = (*trajectories)[trajDay];
  //oddsRatio here is actually the odds ratio of baseline to fever home proportion.
  //This odds ratio remains fixed, and so is used to scale the time spent at home, as below.
  if (mvmtA) {
    // Keep a loc with probability locsProp, but unweighted
    double homeTime = 0;
    double dropTime = 0;
    bool atHomeSome = false;
    bool outSome = false;
    auto itrLoc = todayTraj.begin();
    while (itrLoc != todayTraj.end()) {
      if (houseID == itrLoc->first) {
	homeTime += itrLoc->second;
	++itrLoc;
	atHomeSome = true;
      } else if (rGen->getEventProbability() > locsProp) {
	dropTime += itrLoc->second;
	itrLoc = todayTraj.erase(itrLoc);
      }	else {
	++itrLoc;
	outSome = true;
      }
    ...
    if (!atHomeSome) {
      //never home
      if (outSome) {
	for (auto itrLoc = todayTraj.begin(); itrLoc != todayTraj.end(); ++itrLoc) { 
	  itrLoc->second *= 1/(1-dropTime);
	}
      } else {
	//no remaining locations - use baseline traj
	todayTraj = (*trajectories)[trajDay];
      }
    } else {
      //home some of the time
      if (!outSome) {
	//never out, just 100% at home:
	for (auto itrLoc = todayTraj.begin(); itrLoc != todayTraj.end(); itrLoc++) { 
	  itrLoc->second *= 1/homeTime;
	}
      } else {
	//out some of the time
	//homeProp is the time they should spend at home with fever, homeTime is baseline (not sick)
	double homeProp = homeTime/(homeTime+oddsRatio*(1-homeTime)); 
	for (auto itrLoc = todayTraj.begin(); itrLoc != todayTraj.end(); itrLoc++) { 
	  if (houseID == itrLoc->first) {
	    itrLoc->second *= homeProp/homeTime;
	  } else {
	    itrLoc->second *= (1-homeProp)/(1-homeTime-dropTime);
	  }
	}
      }
    }
    //todayTraj is the trajectory we use for today
  }
}
