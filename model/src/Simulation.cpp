...
string Simulation::readInputs() {
  ...
  for (auto& oRItr : outbreakResponses) {
    oRItr.second->setup(outbreakResponsePrefix + oRItr.first + outbreakResponseSuffix);    
  }
  if (earlyStages){
  ...
  readFeverHumanMvmtFile(feverHumanMvmtFile);
  ...
  for (auto& oRItr : outbreakResponses) { 
    if(oRItr.second->isOutbreakResponseEnabled()){
      if (oRItr.second->outbreakResponseStrategy() == "ring"){
	//This function gets the response radius from the input file, then pairs locations within that radius of each other.
	setRadiusLocations(oRItr.second->getResponseRadius());
      }
      //This function passes the locations to outbreak intervention
      oRItr.second->setLocations(&locations);
    }
  }
  return simName;
}
...
void Simulation::simEngine() {
  ...
  while(currentDay < numDays){
    ...
    for (auto& oRItr : outbreakResponses) {
      oRItr.second->update(currentDay,&rGenControl,&outputReport);
    }
    ...
    outputReport.updateBitesReport(currentDay,bitesToday);
    bitesToday = 0;
    currentDay++;
  }
  ...
  for(auto & phum : humans) {
	...
	if(oRItr.second->isOutbreakResponseEnabled()){
	  ...
	  if(phum.second->isSymptomatic() && floor(phum.second->infection->getSymptomOnset()) == currentDay){
	    ...
	    oRItr.second->addSymptomatic(currentDay, (phum.second).get(), &rGenControl, &outputReport);
    ...
    if (shelterOn
	&& (shelterStart <= currentDay)
	&& (shelterEnd > currentDay)
	&& (shelterCompliance > rGen.getEventProbability())) {
      phum.second->setShelterInPlace();
    } else {
      phum.second->setTrajDay(rGen.getRandomNum(N_TRAJECTORY));
      if (phum.second->isSymptomatic()) {
	if (((currentDay - phum.second->infection->getSymptomOnset()) < stageEndDays.back()) &&
	    (currentDay > phum.second->infection->getSymptomOnset())) {
	  unsigned jj = 0;
	  while (stageEndDays[jj] < (currentDay - phum.second->infection->getSymptomOnset())) {
	    ++jj;
	  }
	  //second parameter calculates odds ratio of baseline odds to new odds of time spent at home
	  //this odds ratio remains fixed
	  phum.second->setFeverEffect(true,
				      baselineHomeProp*(1-homeProportion[jj])/homeProportion[jj]/(1-baselineHomeProp),
				      locsProportion[jj], &rGen);
	} else {
	  phum.second->setFeverEffect(false, 1, 1, &rGen);
	}
      } else {
	phum.second->setFeverEffect(false, 1, 1, &rGen);
      }
...
void Simulation::mosquitoDynamics(){
  ...
  for(auto it = mosquitoes.begin(); it != mosquitoes.end();){
    ...
    double mozHalfDeath = mozHalfDeathBaseline;
    // loop over outbreaks
    // check if resistance loci
    for (auto & oRNItr: outbreakResponseNames) {
      if (std::find(modeledLoci.begin(), modeledLoci.end(), oRNItr) != modeledLoci.end()) {
	auto genotype = it->second->getGenotype(oRNItr);
	if (!(genotype.first || genotype.second)) {
	  mozDeath = locations[it->second->getLocationID()]->getIncreasedMortalityInsecticide(currentDay, mozDeath, oRNItr);
	  mozHalfDeath = locations[it->second->getLocationID()]->getIncreasedMortalityInsecticide(currentDay, mozHalfDeath, oRNItr);
	} else if (!(genotype.first && genotype.second)) {
	  mozDeath = locations[it->second->getLocationID()]->getIncreasedMortalityInsecticide(currentDay, mozDeath, oRNItr,
											      heterozygousProtection[oRNItr]);
	  mozHalfDeath = locations[it->second->getLocationID()]->getIncreasedMortalityInsecticide(currentDay, mozHalfDeath, oRNItr,
												  heterozygousProtection[oRNItr]);
	}
      } else {
    ...
    if(dieTime <= biteTime && dieTime <= 1.0){
      ...
      outputReport.updateMozUniqueBitesReport(currentDay, (it->second).get());
      deathMoz++;
      ++it;
...
void Simulation::generateMosquitoes(){
  ...
  if (earlyStages) {
	...
	//assign mos genotype for each modeled loci
	for (auto genItr = modeledLoci.begin(); genItr != modeledLoci.end(); ++genItr) {
	  double tempRand = rGen.getEventProbability();
	  std::pair<double, double> pupaeGenFreq = x.second->getPupaeGenotypeFrequency(*genItr);
	  if (tempRand < pupaeGenFreq.first) {
	    moz->setGenotype(*genItr, std::make_pair(false, false));
	  } else if (tempRand < (pupaeGenFreq.first + pupaeGenFreq.second)) {
	    moz->setGenotype(*genItr, std::make_pair(true, true));
	  } else {
	    moz->setGenotype(*genItr, std::make_pair(true, false));
	  }
	}
...
void Simulation::implicitEarlyStages(){
  ...
  double maleDeathRate = (currentDay + 1) < mozDailyDeathRate.size() ? mozDailyDeathRate[currentDay] : mozDailyDeathRate.back();
  ...
  double newEggs, newLarvae, newPupae, newMales, larvalCapacity, q_param, r_param, s_param, t_param;
  for (auto& x : locations){
    larvalCapacity = emergenceFactor*emergeFactor*x.second->getLarvalCapacity();
    double oldEggs = x.second->getEggs();
    double oldLarvae = x.second->getLarvae();
    double oldPupae = x.second->getPupae();
    double oldMales = x.second->getMaleAdults();
    newEggs = (gonotrophicCycleRate*mosquitoes.count(x.first) + oldEggs)/(1 + eggDeathRate + eggDevelopmentRate);
    q_param = larvalCapacity*(1+larvalDevelopmentRate+larvaeDeathRate+extraDeathRate)/3;
    r_param = larvalCapacity*(eggDevelopmentRate*newEggs + oldLarvae)/2;
    ...
    newPupae = (larvalDevelopmentRate*newLarvae + oldPupae)/(1 + pupaeDeathRate + pupalDevelopmentRate + extraDeathRate);
    newMales = (pupalDevelopmentRate*newPupae + oldMales)/(1 + maleDeathRate);
    x.second->updateEggs(newEggs); x.second->updateLarvae(newLarvae); x.second->updatePupae(newPupae);
    x.second->updateMaleAdults(newMales);
    outputReport.updateImmatureReport(currentDay, (x.second).get());

    //Loop over genotypes, and update the proportions for each immature stage
    for (auto genItr = modeledLoci.begin(); genItr != modeledLoci.end(); ++genItr) {
      int negMozzes = 0;
      int hetMozzes = 0;
      int posMozzes = 0;
      auto mozRange = mosquitoes.equal_range(x.first);
      for (auto mozItr = mozRange.first; mozItr != mozRange.second; ++mozItr) {
	auto mozTempGen = mozItr->second->getGenotype(*genItr);
	if (mozTempGen.first && mozTempGen.second) {
	  ++posMozzes;
	  outputReport.updateFemaleGeneticsReport(currentDay, *genItr, "resistant");
	} else if (mozTempGen.first || mozTempGen.second) {
	  ++hetMozzes;
	  outputReport.updateFemaleGeneticsReport(currentDay, *genItr, "heterozygous");
	} else {
	  ++negMozzes;
	  outputReport.updateFemaleGeneticsReport(currentDay, *genItr, "susceptible");
	}
      }
      //calculate new egg numbers that are positive and negative
      double negAlleleFrequencyMales = (1 + x.second->getMaleAdultGenotypeFrequency(*genItr).first -
					x.second->getMaleAdultGenotypeFrequency(*genItr).second)/2;
      double negEggsLaid = gonotrophicCycleRate*negAlleleFrequencyMales*(hetMozzes+2*negMozzes)/2;
      double posEggsLaid = gonotrophicCycleRate*(1-negAlleleFrequencyMales)*(hetMozzes+2*posMozzes)/2;
      //double hetEggsLaid = gonotrophicCycleRate*(negAlleleFrequencyMales*posMozzes+hetMozzes/2+(1-negAlleleFrequencyMales)*negMozzes);
      
      
      //solve the three proportions equations
      double newNegEggProp = (x.second->getEggGenotypeFrequency(*genItr).first*oldEggs + negEggsLaid)/(oldEggs + (posMozzes+hetMozzes+negMozzes)*gonotrophicCycleRate);
      double newPosEggProp = (x.second->getEggGenotypeFrequency(*genItr).second*oldEggs + posEggsLaid)/(oldEggs + (posMozzes+hetMozzes+negMozzes)*gonotrophicCycleRate);
      double newNegLarvProp = (x.second->getLarvaeGenotypeFrequency(*genItr).first*oldLarvae + newNegEggProp*eggDevelopmentRate*newEggs)/(oldLarvae + eggDevelopmentRate*newEggs);
      double newPosLarvProp = (x.second->getLarvaeGenotypeFrequency(*genItr).second*oldLarvae + newPosEggProp*eggDevelopmentRate*newEggs)/(oldLarvae + eggDevelopmentRate*newEggs);
      double newNegPupProp = (x.second->getPupaeGenotypeFrequency(*genItr).first*oldPupae + newNegLarvProp*larvalDevelopmentRate*newLarvae)/(oldPupae + larvalDevelopmentRate*newLarvae);
      double newPosPupProp = (x.second->getPupaeGenotypeFrequency(*genItr).second*oldPupae + newPosLarvProp*larvalDevelopmentRate*newLarvae)/(oldPupae + larvalDevelopmentRate*newLarvae);
      double newNegMaleProp = (x.second->getMaleAdultGenotypeFrequency(*genItr).first*oldMales + newNegPupProp*pupalDevelopmentRate*newPupae)/(oldMales + pupalDevelopmentRate*newPupae);
      double newPosMaleProp = (x.second->getMaleAdultGenotypeFrequency(*genItr).second*oldMales + newPosPupProp*pupalDevelopmentRate*newPupae)/(oldMales + pupalDevelopmentRate*newPupae);

      //update location proportions - unless nan
      if ((newNegEggProp < 1) && (newPosEggProp > 0)) {
	x.second->setEggGenotypeFrequency(*genItr, std::make_pair(newNegEggProp, newPosEggProp));
      }
      if ((newNegLarvProp < 1) && (newPosLarvProp > 0)) {
	x.second->setLarvaeGenotypeFrequency(*genItr, std::make_pair(newNegLarvProp, newPosLarvProp));
      }
      if ((newNegPupProp < 1) && (newPosPupProp > 0)) {
	x.second->setPupaeGenotypeFrequency(*genItr, std::make_pair(newNegPupProp, newPosPupProp));
      }
      if ((newNegMaleProp < 1) && (newPosMaleProp > 0)) {
	x.second->setMaleAdultGenotypeFrequency(*genItr,
						std::make_pair(newNegMaleProp, newPosMaleProp));
      }

      outputReport.updateMaleGeneticsReport(currentDay, *genItr, "resistant",
					    newMales
					    * x.second->getMaleAdultGenotypeFrequency(*genItr).second);
      outputReport.updateMaleGeneticsReport(currentDay, *genItr, "susceptible",
					    newMales
					    * x.second->getMaleAdultGenotypeFrequency(*genItr).first);

      outputReport.updateMaleGeneticsReport(currentDay, *genItr, "heterozygous",
					    newMales
					    *(1
					      -x.second->getMaleAdultGenotypeFrequency(*genItr).first
					      -x.second->getMaleAdultGenotypeFrequency(*genItr).second));
...
void Simulation::readSimControlFile(string line) {
  ...
  getline(infile, line, ',');
  feverHumanMvmtFile = line;

  getline(infile, line, ',');
  string geneticsFile = line;

  getline(infile, line, ',');
  string shelterFile = line;
  
  readGeneticsFile(geneticsFile);
  readInitialFOI(foiFile);
  readDailyFOI(dailyFoiFile);
  readDailyLocalFOI(dailyResidentsFoiFile);
  readOutbreakFile(outbreakFile);
  readShelterFile(shelterFile);
}

void Simulation::readOutbreakFile(string file) {
    if (file.length() == 0)
    {
      std::cout << "outbreak file has length 0" << std::endl;
      exit(1);
    }
  string line;
  std::ifstream infile(file);
  if(!infile.good())
    {
      std::cout << "outbreak file is not good" << std::endl;
      exit(1);
    }
  ...
  infile.close();
  this->readParameter("outbreak_response_names", &outbreakResponseNames);
  for (auto oRItr = outbreakResponseNames.begin(); oRItr != outbreakResponseNames.end(); ++oRItr) {
    unique_ptr<OutbreakResponse> oR(new OutbreakResponse(*oRItr));
    outbreakResponses.insert(make_pair(*oRItr, move(oR)));
  }
  this->readParameter("outbreak_response_suffix", &outbreakResponseSuffix);
  this->readParameter("outbreak_response_prefix", &outbreakResponsePrefix);
}

void Simulation::readShelterFile(string file) {
    if (file.length() == 0)
    {
      std::cout << "shelter file has length 0" << std::endl;
      exit(1);
    }
  string line;
  std::ifstream infile(file);
  if(!infile.good())
    {
      std::cout << "shelter file is not good" << std::endl;
      exit(1);
    }
  ...
  infile.close();
  shelterOn = this->readParameter("shelter_in_place", false);
  shelterStart = this->readParameter("start_day", 0);
  shelterEnd = this->readParameter("end_day", 0);
  shelterCompliance = this->readParameter("compliance", 0.0);
}
    ...
    if (earlyStages){
      ...
      unique_ptr<Location> location(new Location(locID, locType, mohID, x, y, 
						 ...
						 normalizedLarvalCapacity*std::pow(mozzes, 
										   larvalPower),
						 initializationFactor,
						 outbreakResponseNames));
      //set early stage genotypes:
      for (auto genItr = modeledLoci.begin(); genItr != modeledLoci.end(); ++genItr) {
	auto initialProps = std::make_pair(initialHomozygousNegative[*genItr],
					   initialHomozygousPositive[*genItr]);
	location->setEggGenotypeFrequency(*genItr,initialProps);
	location->setLarvaeGenotypeFrequency(*genItr,initialProps);
	location->setPupaeGenotypeFrequency(*genItr,initialProps);
	location->setMaleAdultGenotypeFrequency(*genItr, initialProps);
      }
      ...
      for(int i = 0; i < initialMozCount; i++){
        ...
        //set genotype:
	for (auto genItr = modeledLoci.begin(); genItr != modeledLoci.end(); ++genItr) {
	  std::pair<bool, bool> genotype;
	  auto tempRand = rGen.getEventProbability() - initialHomozygousNegative[*genItr];
	  if (tempRand < 0) {
	    genotype = std::make_pair(false, false);
	  } else if (tempRand < initialHomozygousPositive[*genItr]) {
	    genotype = std::make_pair(true, true);
	  } else {
	    genotype = std::make_pair(true, false);
	  }
	  moz->setGenotype(*genItr,genotype);
	}
...
void Simulation::readFeverHumanMvmtFile(string file) {
  if (file.length() == 0)
    {
      std::cout << "Fever Human movement file has length 0" << std::endl;
      exit(1);
    }
  string line;
  std::ifstream infile(file);
  if(!infile.good())
    {
      std::cout << "Fever Human movement file is not good" << std::endl;
      exit(1);
    }
  ...
  this->readParameter("stage_end_days", &stageEndDays);
  this->readParameter("home_proportion", &homeProportion);
  this->readParameter("locs_proportion", &locsProportion);
  baselineHomeProp = this->readParameter("baseline_home_prop", 0.6);
}

void Simulation::readGeneticsFile(string file) {
  if (file.length() == 0)
    {
      std::cout << "Genetics file has length 0" << std::endl;
      exit(1);
    }
  string line;
  std::ifstream infile(file);
  if(!infile.good())
    {
      std::cout << "Genetics file is not good" << std::endl;
      exit(1);
    }
  ...
  this->readParameter("loci_of_interest", &modeledLoci);

  //Next two lines create vector and function for zipping other parameters into a map keyed by the loci.
  std::vector<double> tempVector;
  auto zip = [](const std::string &s, double d) {return std::make_pair(s, d); };
  
  this->readParameter("heterozygous_protection", &tempVector);
  if (tempVector.size() != modeledLoci.size()) {
    std::cout << "There is an inconsistent number of entries" << std::endl;
    exit(1);
  }
  std::transform(modeledLoci.begin(), modeledLoci.end(), tempVector.begin(),
		 std::inserter(heterozygousProtection, heterozygousProtection.end()), zip);

  this->readParameter("initial_homozyg_neg", &tempVector);
  if (tempVector.size() != modeledLoci.size()) {
    std::cout << "There is an inconsistent number of entries" << std::endl;
    exit(1);
  }
  std::transform(modeledLoci.begin(), modeledLoci.end(), tempVector.begin(),
		 std::inserter(initialHomozygousNegative, initialHomozygousNegative.end()), zip);

  this->readParameter("initial_homozyg_pos", &tempVector);
  if (tempVector.size() != modeledLoci.size()) {
    std::cout << "There is an inconsistent number of entries" << std::endl;
    exit(1);
  }
  std::transform(modeledLoci.begin(), modeledLoci.end(), tempVector.begin(),
		 std::inserter(initialHomozygousPositive, initialHomozygousPositive.end()), zip);
...
void Simulation::parseVector(string line, vector<unsigned> * vector_temp){
  std::stringstream linetemp;
  string line2;
  linetemp.clear();
  linetemp << line;
...
void Simulation::parseVector(string line, vector<string> * vector_temp){
  std::stringstream linetemp;
  string line2;
  linetemp.clear();
  linetemp << line;
...
void Simulation::readParameter(string param_name, vector<double> * vtemp) {
  map<string, string>::iterator it;
  it = parameters.find(param_name);
  if(it != parameters.end()){
    this->parseVector(it->second, vtemp);
  }
}

void Simulation::readParameter(string param_name, vector<unsigned> * vtemp) {
  map<string, string>::iterator it;
  it = parameters.find(param_name);
  if(it != parameters.end()){
    this->parseVector(it->second, vtemp);
...
void Simulation::readParameter(string param_name, vector<string> * vtemp) {
  map<string, string>::iterator it;
  it = parameters.find(param_name);
  if(it != parameters.end()){
    this->parseVector(it->second, vtemp);
